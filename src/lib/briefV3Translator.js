...
//
// PHASE 1A: only sections 1-4 are wired with prompts. Sections 5-22
// fall through to a placeholder content that the renderer surfaces
// as "Coming in next phase". That lets us ship the new shell on real
// briefs immediately without waiting on the full prompt library.
//
...
import {
  BRIEF_V3_SECTIONS,
  BRIEF_V3_PHASE_1A_KEYS,
  BRIEF_V3_SCHEMA_VERSION,
  scrubDashesV3,
  emptyV3ContentForShape,
...
export async function translateBriefV3(briefText, { onSection } = {}) {
  console.log('[translateBriefV3] start. brief length:', briefText?.length || 0, 'sections:', BRIEF_V3_SECTIONS.length, 'wired:', BRIEF_V3_PHASE_1A_KEYS.length)
  ...
  const WAVE_SIZE = 4
  const wiredSections = BRIEF_V3_SECTIONS.filter(s => BRIEF_V3_PHASE_1A_KEYS.includes(s.key))
