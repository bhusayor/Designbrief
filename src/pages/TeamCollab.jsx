...
import {
  ...
  SwatchIcon, UsersIcon, EnvelopeIcon, UserPlusIcon,
...
import { sendInvite } from '../lib/inviteApi'
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* ── Team tab ── */}
        {activeTab === 'team' && <TeamSection teamMembers={teamMembers} invites={invites} workspace={workspace} onInviteClick={() => setShowTeamModal(true)} />}
...
const TEAM_AVATAR_PALETTE = ['#7C3AED', '#0EA5E9', '#16a34a', '#f59e0b', '#EC4899', '#6366F1', '#0891B2']

function TeamAvatar({ name, size = 34 }) {
  const initials = (name || '?').split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)
  const color = TEAM_AVATAR_PALETTE[(name?.charCodeAt(0) || 0) % TEAM_AVATAR_PALETTE.length]
  return (
    <div style={{
      ...
      fontSize: Math.round(size * 0.35), color, flexShrink: 0, letterSpacing: '-0.5px',
    }}>
      {initials}
    </div>
  )
...
function TeamRoleBadge({ role }) {
  ...
  }[role?.toLowerCase()] || { color: 'var(--color-text-muted)', bg: 'var(--color-surface)', border: 'var(--color-border)' }
  return (
    <span style={{
      fontFamily: 'var(--font-mono)', fontSize: 9, fontWeight: 700,
...
function TeamSection({ teamMembers, invites, workspace, onInviteClick }) {
  ...
  async function handleSend() {
    ...
    if (!workspace?.id) { setError('No workspace found.'); return }
  ...
  const TABS = [
    { id: 'members', label: 'Members', count: teamMembers.length, Icon: UsersIcon },
    ...
    <div style={{ flex: 1, overflowY: 'auto', fontFamily: 'var(--font-sans)' }}>
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '24px 24px 40px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            <div style={{ fontWeight: 800, fontSize: 18, letterSpacing: '-0.03em', color: 'var(--color-text)', marginBottom: 3 }}>
              {workspace?.name || 'Team'}
            </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <select
              ...
              style={{
                ...
                fontFamily: 'var(--font-sans)', fontSize: 13,
                color: 'var(--color-text)', outline: 'none', cursor: 'pointer', flexShrink: 0,
              }}
            >
            ...
            <button
              ...
              onMouseEnter={e => { if (!sending && email.trim()) e.currentTarget.style.background = '#6D28D9' }}
              onMouseLeave={e => { if (!sending && email.trim()) e.currentTarget.style.background = '#7C3AED' }}
            >
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
            <span style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>
              <strong style={{ color: 'var(--color-text-muted)', fontWeight: 600 }}>Member</strong> — view and work on projects
            </span>
            <span style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>
              <strong style={{ color: 'var(--color-text-muted)', fontWeight: 600 }}>Admin</strong> — invite others, manage workspace
            </span>
          </div>
          {error && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 8, padding: '7px 10px', background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 8 }}>
              <ExclamationCircleIcon style={{ width: 13, height: 13, color: '#DC2626', flexShrink: 0 }} />
              <span style={{ fontSize: 12, color: '#DC2626' }}>{error}</span>
            </div>
          )}
          {success && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 8, padding: '7px 10px', background: 'rgba(22,163,74,0.06)', border: '1px solid rgba(22,163,74,0.2)', borderRadius: 8 }}>
          ...
          {TABS.map(t => (
            <button
              ...
              style={{
                ...
                borderBottom: '2px solid ' + (teamTab === t.id ? '#7C3AED' : 'transparent'),
                marginBottom: -1,
                cursor: 'pointer', fontFamily: 'var(--font-sans)', fontSize: 13,
                fontWeight: teamTab === t.id ? 700 : 500,
                color: teamTab === t.id ? 'var(--color-text)' : 'var(--color-text-muted)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
            ...
            >
              ...
              {t.count > 0 && (
                <span style={{
                  background: teamTab === t.id ? '#7C3AED' : 'var(--color-surface)',
                  color: teamTab === t.id ? '#fff' : 'var(--color-text-muted)',
        ...
        {teamTab === 'members' && (
          teamMembers.length === 0 ? (
            <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--color-text-muted)', fontSize: 13 }}>
              <UsersIcon style={{ width: 28, height: 28, margin: '0 auto 10px', display: 'block', opacity: 0.3 }} />
              No team members yet. Use the chat to build your team.
            </div>
          ) : (
            <div style={{ border: '1px solid var(--color-border)', borderRadius: 14, overflow: 'hidden', marginTop: 12 }}>
              {teamMembers.map((member, i) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '11px 16px',
                    borderBottom: i < teamMembers.length - 1 ? '1px solid var(--color-border)' : 'none',
                    transition: 'background 0.1s',
                  }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-surface)' }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'transparent' }}
                >
                  <TeamAvatar name={member.name || member.role} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--color-text)', marginBottom: 1 }}>
        ...
        {teamTab === 'pending' && (
          pendingInvites.length === 0 ? (
            <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--color-text-muted)', fontSize: 13 }}>
              <EnvelopeIcon style={{ width: 28, height: 28, margin: '0 auto 10px', display: 'block', opacity: 0.3 }} />
              No pending invites
            </div>
          ) : (
            <div style={{ border: '1px solid var(--color-border)', borderRadius: 14, overflow: 'hidden', marginTop: 12 }}>
              {pendingInvites.map((invite, i) => (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--color-text)', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {invite.invitee_email || invite.invited_email}
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', gap: 5 }}>
                      <ClockIcon style={{ width: 10, height: 10 }} />
                      {invite.expires_at ? daysUntil(invite.expires_at) : 'Pending'}
                      {invite.created_at && <><span style={{ opacity: 0.4 }}>·</span> Sent {timeAgo(invite.created_at)}</>}
                    </div>
                  </div>
                  <span style={{
                    fontFamily: 'var(--font-mono)', fontSize: 9, fontWeight: 700,
                    color: '#f59e0b', background: '#FEF9C3', border: '1px solid #FDE68A',
                    borderRadius: 100, padding: '2px 8px',
                    textTransform: 'uppercase', letterSpacing: '0.06em',
                  }}>
                    {invite.job_role || invite.role || 'member'}
                  </span>
                </div>
              ))}
            </div>
